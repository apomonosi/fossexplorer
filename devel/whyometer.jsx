...
export default function Whyometer() {
  ...
  const render = useCallback(() => {
    ...
    // Track arcs (background rings for each reason)
    REASONS.forEach((reason, i) => {
      ...
      g.append("circle")
        ...
        .attr("fill", "none")
        .attr("stroke", "#111827")
        .attr("stroke-width", bandWidth - 3)
        .attr("stroke-dasharray", `${2 * Math.PI * r}`)
        .attr("opacity", 0.6);
    ...
    // Create arc generators and paths for each reason
    const arcPaths = REASONS.map((reason, i) => {
      ...
      arcsGroup.append("path")
        ...
        .attr("fill", reason.color)
        .attr("opacity", 0.06);

      // Value arc
      const valuePath = arcsGroup.append("path")
        .attr("class", `arc-${i}`)
        .attr("fill", reason.color)
        .attr("opacity", 0.7)
      ...
      // Bright tip
      const tipPath = arcsGroup.append("path")
        .attr("class", `tip-${i}`)
        .attr("fill", reason.color)
        .attr("opacity", 0.95);
    ...
    REASONS.forEach((reason, i) => {
      const angle = -Math.PI * 0.75 + (Math.PI * 1.5) * (reason.base / 100);
      const r = innerR + 16 + i * bandWidth;
      const labelR = outerR + 24;
      const segAngle = (-Math.PI * 0.75 + Math.PI * 0.75) / 2 + ((i / (n - 1)) - 0.5) * Math.PI * 0.6;

      // Label at fixed positions around the outside
      const lAngle = -Math.PI + ((i + 0.5) / n) * Math.PI * 2;
      const lx = Math.cos(lAngle) * (outerR + 36);
      const ly = Math.sin(lAngle) * (outerR + 36);
    });
    ...
    for (let i = 0; i < 24; i++) {
      ...
      g.append("circle")
        .attr("cx", Math.cos(a) * (innerR + 1))
        .attr("cy", Math.sin(a) * (innerR + 1))
        .attr("r", 1.2)
        .attr("fill", "#334155");
    }
    ...
    const centerVal = g.append("text")
      ...
      .attr("y", -10)
      .attr("fill", "#22d3ee")
      .attr("font-size", innerR * 0.48)
      .attr("font-weight", 700)
      .attr("font-family", "'Oxanium', 'JetBrains Mono', monospace")
    ...
    const pctSign = g.append("text")
      ...
      .attr("dominant-baseline", "central")
      .attr("y", -10 - innerR * 0.08)
      .attr("fill", "#475569")
      .attr("font-size", innerR * 0.18)
      .attr("font-family", "'Oxanium', monospace");

    // Scale labels at -135deg (0%) and +135deg (100%)
    const scaleR = outerR + 18;
    const startA = -Math.PI * 0.75;
    const endA = Math.PI * 0.75;

    [0, 25, 50, 75, 100].forEach(v => {
      const a = startA + (endA - startA) * (v / 100);
      const x = Math.cos(a) * (scaleR + 4);
      const y = Math.sin(a) * (scaleR + 4);
      g.append("text")
        .attr("x", x).attr("y", y)
        .attr("text-anchor", "middle")
        .attr("dominant-baseline", "central")
    ...
    function updateArcs(values) {
      ...
      arcPaths.forEach(({ arcGen, valuePath, tipPath }, i) => {
        ...
        // bright tip
        const tipGen = d3.arc()
          .innerRadius(arcGen.innerRadius()())
          .outerRadius(arcGen.outerRadius()())
  ...
  return (
    <div style={{
      ...
      {/* Title */}
      <header style={{ textAlign: "center", padding: "36px 24px 8px" }}>
        <h1 style={{
